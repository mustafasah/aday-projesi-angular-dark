...
@Component({
  selector: 'TeklifGorusmesi',
  templateUrl: './teklif-gorusmesi.component.html',
  styleUrls: ['./teklif-gorusmesi.component.scss']
})
export class TeklifGorusmesiComponent implements OnInit {
  @Input() candidate: any
  ...
  ngOnInit() {
    ...
    this.firebaseService.getTklfQuestions().snapshotChanges().pipe(
      map(changes =>
        changes.map(c =>
  ...
  backStep(textvalue) {
    ...
    this.camundaService.completeTask(this.candidate.taskId, "teknik_gorusme_yeniden_yapilsin").pipe(
      tap(() => {
        this.comp.ngOnInit()
  ...
  nextStep(textvalue) {
    this.camundaService.setCommentByTaskId(this.candidate.taskId, textvalue).subscribe()
    this.camundaService.completeTask(this.candidate.taskId, "teklif_verilenler").pipe(
      tap(() => {
        this.comp.ngOnInit()
        this.comp.ngAfterViewInit()
      })).subscribe()
  ...
  teknik_yetersiz(){
    let r = confirm("Emin misiniz?");
    if (r == true) {
      this.camundaService.completeTask(this.candidate.taskId, "teknik_yetersiz").pipe().subscribe(res => {
        this.toastr.success('<span class="tim-icons icon-bell-55" [data-notify]="icon"></span><b>Başarılı!</b> Listeye Eklendi', '', {
          disableTimeOut: false,
          closeButton: true,
          enableHtml: true,
