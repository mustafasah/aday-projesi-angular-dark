...
@Component({
  selector: 'AdayIstifasi',
  templateUrl: './aday-istifasi.component.html',
  styleUrls: ['./aday-istifasi.component.scss']
})
export class AdayIstifasiComponent implements OnInit {
  @Input() candidate: any;
  ...
  nextStep2(textvalue) {
    this.camundaService.setCommentByTaskId(this.candidate.taskId, textvalue).subscribe()
    this.camundaService.completeTask(this.candidate.taskId, "adayi_denemeye_al").pipe(
      tap(() => {
        this.comp.ngOnInit()
        this.comp.ngAfterViewInit()
      })).subscribe()
