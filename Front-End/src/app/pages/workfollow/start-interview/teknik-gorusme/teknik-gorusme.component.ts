...
@Component({
  selector: 'TeknikGorusme',
  templateUrl: './teknik-gorusme.component.html',
  styleUrls: ['./teknik-gorusme.component.scss']
})
export class TeknikGorusmeComponent implements OnInit {
  @Input() candidate: any
  ...
  ngOnInit() {
    ...
    this.firebaseService.getTknkQuestions().snapshotChanges().pipe(
      map(changes =>
        changes.map(c =>
  ...
  backStep(textvalue) {
    ...
    this.camundaService.completeTask(this.candidate.taskId, "ik_gorusmesi_bastan_yapilsin").pipe(
      tap(() => {
        this.comp.ngOnInit()
  ...
  nextStep(textvalue) {
    this.camundaService.setCommentByTaskId(this.candidate.taskId, textvalue).subscribe()
    this.camundaService.completeTask(this.candidate.taskId, "teknik_olarak_uygun").pipe(
      tap(() => {
        this.comp.ngOnInit()
        this.comp.ngAfterViewInit()
      })).subscribe()
